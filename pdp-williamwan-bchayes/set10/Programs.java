...
public class Programs {
	...
	public static ExpVal run (List<Def> pgm, List<ExpVal> inputs) {
		...
		Map<String,ExpVal> initialEnv = new HashMap<String,ExpVal>();
		
		// if List<Def> has more than one element process first element, 
		// otherwise throw error that there are no Def.
		if (pgm.size()>0) {
			
			// debug info, prints out AST structure for each Def
			ProgramsH.outputPgm(pgm);
			
			// load all Def into initialEnv, this is tricky, because as we add
			// each function, the environment associated to the funval is 
			// limited to what has been previously added. So Def 0 in the list
			// would be added with an empty environment, def 1 would have 1
			// element, etc. So we would to iterate through the list N times 
			// to make sure every element has an updated environment.
			for(int i=0; i< pgm.size(); i++) {
				for(Def d : pgm) {
					if (d.rhs().isLambda()) {
						initialEnv.put(d.lhs().toString(), Asts.expVal(d.rhs().asLambda(), initialEnv));
					} else if (d.rhs().isConstant()) {
						initialEnv.put(d.lhs(), d.rhs().asConstant().value());
					}
				}
			}
			...
			return  processFirstDef(pgm.get(0), pgm, inputs, initialEnv);
	...
	private static ExpVal processFirstDef( Def d, 
									  List<Def> pgm, 
									  List<ExpVal> inputs, 
		...
		// make copy of starting map as newMap
		...
		if (d.rhs()==null) {
			...
			if (d.rhs().isLambda()) {
				...
				for(int i=0;i<inputs.size();i++) {
					...
					// add lambda formals to newMap with the given inputs as values
					newMap.put(lambda.formals().get(i).toString(), inputs.get(i));
				}
				
				ProgramsH.outputMap(newMap);
				
				// process lambda
				result = lambda.body().value(newMap);
				
				// check if result is a FunVal
				if (result.isFunction()) {
					result = result.asFunction().code().body().value(newMap);
				}
