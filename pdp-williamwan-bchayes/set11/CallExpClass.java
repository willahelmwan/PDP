...
class CallExpClass extends ExpClass implements CallExp {
	...
	public ExpVal value(Map<String,ExpVal> env) {
		...
		// copy env
		Map<String, ExpVal> operEnv = new TreeMap<String, ExpVal>(env);
		...
		if (operResult.isFunction()) {
			...
			fvEnv = new TreeMap<String,ExpVal>(operResult.asFunction().environment());
			...
			for(int i=0; i<this.arguments().size();i++) {
				...
				argEnv = new TreeMap<String,ExpVal>(env);
