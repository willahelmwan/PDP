...
class ProgramsH {
	...
	//         expressions value
	public static Map<String,ExpVal> setKeyToExpValue(
													String key,
													Exp exp,
		...
		// constant
		if (exp.isConstant()) {
			env = ProgramsH.extend(env, key, ((ConstantExp) exp).value(env));
		// lambda
		} else if (exp.isLambda()) {
			env = ProgramsH.extend(env, key, ((LambdaExp) exp).value(env));
		// identifier	
		} else if (exp.isIdentifier()) {
			env = ProgramsH.extend(env, key, ((IdentifierExp) exp).value(env));
		// arithmetic	
		} else if (exp.isArithmetic()) {
			env = ProgramsH.extend(env, key, ((ArithmeticExp) exp).value(env));
		// call	
		} else if (exp.isCall()) {
			env = ProgramsH.extend(env, key, ((CallExp) exp).value(env));
		// if	
		} else if (exp.isIf()) {
			env = ProgramsH.extend(env, key, ((IfExp) exp).value(env));
		}
	...
	public static String getExpKey(Exp exp, Map<String, ExpVal> env) {
	...
	// extend 
	// GIVEN: a Map, key and value
	// RETURNS: a clone of the Map with the new key/value added
	public static Map<String, ExpVal> extend(Map<String, ExpVal> env, 
												String key,
												ExpVal value) {
		
		Map<String, ExpVal> newEnv = new HashMap(env);
