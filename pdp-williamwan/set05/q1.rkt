...
;; Now use higher-order functions whenever possible instead of using recursions
;; to traverse lists.
...
(define qvy4 -9)
;; Star's parameters
(define PC 8) ; Radial star's point-count
(define RS-inner 10) ; Radial star's inner radius
(define RS-outer 50) ; Radial star's outer radius
(define RS-outline "solid") ; Radial star's outline mode

;; Square's parameters
(define SQ-side 71) ; Square's side length
(define SQ-outline "solid") ; Square's outline mode
...
(define (doodad-list-after-button-down lst mx my)
  (map
   ;; Doodad -> Doodad
   ;; RETURNS: the Doodad following a button-down at the given location.
   (lambda (d) (doodad-after-button-down d mx my))
   lst
   ))
...
(define (doodad-list-after-drag lst mx my)
  (map
   ;; Doodad -> Doodad
   ;; RETURNS: the Doodad following a drag key event.
   (lambda (d) (doodad-after-drag d mx my))
   lst
   ))
...
(define (doodad-list-after-button-up lst mx my)
  (map
   ;; Doodad -> Doodad
   ;; RETURNS: the Doodad following a button-up key event.
   (lambda (d) (doodad-after-button-up d mx my))
   lst
   ))
...
;; starlist-to-scene : LOD -> Scene
;; GIVEN: a List of Doodads
;; RETURNS: a Scene that puts all Doodads in the list on the Scene
;; EXAMPLE:
;; (starlist-to-scene star-list) =
...
(define (starlist-to-scene lst)
  (foldr
   ;; Doodad Scene -> Scene
   ;; GIVEN: A Doodad and a Scene
   ;; RETURNS: A Scene with the Doodad placed in the given Scene. 
   (lambda (d s) (place-image (radial-star PC RS-inner RS-outer RS-outline
                                           (doodad-color d))
...
(define (star-to-scene w)
  (starlist-to-scene (world-doodads-star w)))
...
;; squarelist-to-scene : LOD World -> Scene
;; GIVEN: a List of Doodads and a World
;; RETURNS: a Scene that puts all Doodads in the list on the Scene with all the
;;          star Doodads from the world on the Scene already.
;; EXAMPLE:
;; (squarelist-to-scene square-list ex-world3) =
;;       (place-image (square 71 "solid" "orange") 1 225 EMPTY-CANVAS)
;; STRATEGY: Use HOF foldr on lst

(define (squarelist-to-scene lst w)
  (foldr
   ;; Doodad Scene -> Scene
   ;; GIVEN: a Doodad and a Scene
   ;; RETURNS: A Scene with the given Doodad placed in it. 
   (lambda (d s) (place-image (square SQ-side SQ-outline
                                      (doodad-color d))
                              (doodad-x d) (doodad-y d) s))
   (star-to-scene w)
   lst))
...
(define (square-to-scene w)
  (squarelist-to-scene (world-doodads-square w) w))
...
(define (selected-doodad lst)
  (filter
   ;; Doodad -> Boolean
   ;; RETURNS: true iff the Doodad is selected.
   (lambda (d) (doodad-selected? d))
   lst))

;; TEST:
(begin-for-test
  (check-equal?
    (selected-doodad LOD-square2)
    (list (make-doodad 1 225 -13 9 "orange" #true "square" 8 9 9)
          (make-doodad 300 1 13 -9 "khaki" #true "square" 5 7 6))
    "(selected-doodad LOD-star1) returned incorrect list"))
...
(define (world-to-scene w)
  (cond
    [(not (empty? (selected-doodad (world-doodads-star w))))
     (place-image (circle 3 "solid" "black")
                  (+ (doodad-x (first (selected-doodad
                                       (world-doodads-star w))))
                     (doodad-dx (first (selected-doodad
                                        (world-doodads-star w)))))
                  (+ (doodad-y (first (selected-doodad
                                       (world-doodads-star w))))
                     (doodad-dy (first (selected-doodad
                                        (world-doodads-star w)))))
                  (square-to-scene w))]
    [(not (empty? (selected-doodad (world-doodads-square w))))
     (place-image (circle 3 "solid" "black")
                  (+ (doodad-x (first (selected-doodad
                                       (world-doodads-square w))))
                     (doodad-dx (first (selected-doodad
...
;; older : NonNegInteger NonNegInteger -> NonNegInteger
;; GIVEN: two ages of two Doodads
;; RETURNS: the older one of the two, if equal then return the first number
;; EXAMPLE:
;; (older 0 1) = 1
;; (older 2 2) = 2
;; STRATEGY: combine simpler functions
...
;; GIVEN: a List of Doodads
;; RETURNS: the age of the oldest Doodad or Doodads, if the list empty then
;;          return 0.
;; EXAMPLE:
;; (oldest-doodad-age LOD-star1) = 3
;; (oldest-doodad-age LOD-star2) = 7
;; STRATEGY: Use HOF foldr and map on lst

(define (oldest-doodad-age lst)
  (foldr older 0 (map doodad-age lst)))
...
(define (doodad-remove lst n)
  (filter
   ;; Doodad -> Boolean
   ;; RETURNS: true iff the age of the Doodad is not equal to n
   (lambda (n1) (not (= (doodad-age n1) n)))
   lst))
...
;; doodad-color-change : Doodad -> Doodad
;; GIVEN: a Doodad
;; RETURNS: a Doodad with a color change if the Doodad is selected
;;          or the original Doodad is the Doodad is not selected.
;; EXAMPLE:
;; (doodad-color-change (make-doodad 300 225 10 12 "green" #true "star" 3 8 7))
;;     = (make-doodad 300 225 10 12 "blue" #true "star" 3 8 7)
;; (doodad-color-change (make-doodad 125 120 10 12 "gold" #false "star" 0 0 0))
;;     = (make-doodad 125 120 10 12 "gold" #false "star" 0 0 0)
;; STRATEGY: Use template of Doodad on d

(define (doodad-color-change d)
  (if (doodad-selected? d)
      (make-doodad (doodad-x d) (doodad-y d)
